...
peru_edges <- peru_ibd %>% left_join(peru_nodes %>% select(sample_id, id, date), by = c("sampleid1" = "sample_id")) %>% rename(id.x = id, date2 = date)

peru_edges <- peru_edges %>% left_join(peru_nodes %>% select(sample_id, id,date), by = c("sampleid2" = "sample_id")) %>% rename(id.y = id, date1 = date)
```
...
```{r}
save.image("intermediate_data/Dcifer_IBD.RData")
```
