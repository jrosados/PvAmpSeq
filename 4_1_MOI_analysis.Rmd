...
##Correlation age and MOI Supp_figure5
```{r, echo=FALSE}
...
age_moi_SI<- hap_SI_epi %>% 
  ...
  ggplot() +
  geom_point(aes(x = age_y, y = max_moi),
             size = 2, alpha = 0.8) +
...
print(age_moi_SI)
cor.test(age_moi_SI$data$age_y, age_moi_SI$data$max_moi)
...
age_moi_PE<- hap_PE_epi %>% 
  ...
  ggplot() +
  geom_point(aes(x = age, y = max_moi),
             size = 2, alpha = 0.8) +
  ggtitle("Peru") +
  ylab("max MOI at the baseline") + xlab(" age (y)") +
  theme_classic(base_size = 12) + 
...
cor.test(age_moi_PE$data$age, age_moi_PE$data$max_moi)
...
Supp_figure5<-ggarrange(age_moi_SI, age_moi_PE,
                              ncol=2, nrow=1, common.legend = F, legend="none")

print(Supp_figure5)

ggsave("figs/Supp_figure5.tiff", units="cm", width=12, height=8.12, dpi=300, compression = 'lzw')


```

##Distribution Symp_Asymp and MOI Supp_figure6
```{r, echo=FALSE}
...
Supp_figure6<-ggarrange(symp_moi_SI_hist, symp_moi_PE_hist,
                              ncol=2, nrow=1, common.legend = TRUE, legend="bottom")

print(Supp_figure6)

ggsave("figs/Supp_figure6.tiff", units="cm", width=12, height=8.12, dpi=300, compression = 'lzw')
...
days_recurr_moi_SI<- hap_SI_epi %>% 
  ...
  group_by(info) %>% 
  mutate(delay_since_prev_ep = as.numeric( vis_date - dplyr::first(vis_date), 
                                                   units = "days", na.rm=T),
...
days_recurr_moi_PE<- hap_PE_epi %>% 
  ...
  mutate(delay_since_prev_ep = as.numeric( date - dplyr::first(date), 
                                                   units = "days", na.rm=T),
...
days_recurr_change_moi_SI<- hap_SI_epi %>% 
  ...
  mutate(delay_since_prev_ep = as.numeric( vis_date - dplyr::first(vis_date), 
                                                   units = "days", na.rm=T),
        change_moi = as.numeric(max_moi -lag(max_moi, default = dplyr::first(max_moi))),
         site = "Solomon Islands",
...
days_recurr_change_moi_plot <-
  ...
  geom_point(aes(x = delay_since_prev_ep, y = change_moi, color= treatment),
             size = 2, alpha = 0.8) +
  ggtitle("") +
  ylab("Change in MOI between\nrecurrent infections") + xlab("days since preceding infection") +
  scale_fill_brewer(palette="Paired") +
  theme_classic(base_size = 12) + 
...
ggsave("figs/Supp_figure7.tiff", units="cm", width=25.14, height=18.12, dpi=300, compression = 'lzw')

```

###Tables days recurrence and MOI
...
wilcox.test(days_recurr_change_moi_PE$delay_since_prev_ep ~ days_recurr_change_moi_PE$clones, alternative = "two.sided")

print(table_days_recurr_change_moi_PE,showAllLevels = TRUE,
      nonnormal = "delay_since_prev_ep")

kruskal.test(days_recurr_change_moi_PE$delay_since_prev_ep ~ days_recurr_change_moi_PE$change)
...
wilcox.test(days_recurr_change_moi_SI$delay_since_prev_ep ~ days_recurr_change_moi_SI$clones, alternative = "two.sided")


print(table_days_recurr_change_moi_SI,showAllLevels = TRUE,
      nonnormal = "delay_since_prev_ep")

kruskal.test(days_recurr_change_moi_SI$delay_since_prev_ep ~ days_recurr_change_moi_SI$change)
```
...
wilcox.test(MOI_day0_dayX_paired_SI$data$max_moi ~ MOI_day0_dayX_paired_SI$data$day, alternative = "two.sided")
...
wilcox.test(MOI_day0_dayX_paired_PE$data$max_moi ~ MOI_day0_dayX_paired_PE$data$day, alternative = "two.sided")
