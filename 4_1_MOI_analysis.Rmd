...
age_moi_SI<- hap_SI_epi %>% 
  ...
  ggplot(aes(x = as.factor(max_moi), y = age_y )) +
  geom_violin()+
  geom_point(size = 2, alpha = 0.4, position = position_jitterdodge()) +
  ggtitle("Solomon Islands") +
  xlab("max MOI at the baseline") + ylab(" age (y)") +
  theme_classic(base_size = 12) + 
  theme(axis.text.x = element_text(face = "plain", color="black", size = 8), legend.position = "none") 
...
age_moi_PE<- hap_PE_epi %>% 
  ...
  ggplot(aes(x = as.factor(max_moi), y = age )) +
  geom_violin()+
  geom_point(size = 2, alpha = 0.4,position = position_jitterdodge()) +
  ggtitle("Peru") +
  xlab("max MOI at the baseline") + ylab(" age (y)") +
