...
analysis_data_SI <- get_analysis_data_sols(random_pairs)

fs <- analysis_data_SI %>% 
  calculate_freqs() %>% 
  pluck("fs")

test <- compute_null(analysis_data_SI, fs, gv)
...
marginal_results <- map_dfr(1:N_sim, function(i) {
  ...
  results <- compute_null(analysis_data_SI, fs, gv)
...
analysis_data_PE <- get_analysis_data_peru(random_pairs)
...
test <- compute_null(analysis_data_PE, fs, gv)
...
marginal_results <- map_dfr(1:N_sim, function(i) {
  ...
  results <- compute_null(analysis_data_PE, fs, gv)
