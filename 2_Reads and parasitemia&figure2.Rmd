...
# Calculate correlation between parasites_ul and count stratified by marker_id

# Perform correlation analysis stratified by marker_id
correlation_results_SI <- reads_parasite_per_marker_SI %>%
  group_by(marker_id) %>%
  summarise(correlation = cor(parasites_ul, count, use = "complete.obs"),
            p_value = cor.test(parasites_ul, count, method = "pearson")$p.value)

# Adjust p-values using the Benjamini-Hochberg method (FDR)
correlation_results_SI$adjusted_p_value <- p.adjust(correlation_results_SI$p_value, method = "fdr")

range(correlation_results_SI$adjusted_p_value)
range(correlation_results_SI$correlation)
...
# Perform correlation analysis stratified by marker_id
correlation_results_PE <- reads_parasite_per_marker_PE %>%
  group_by(marker_id) %>%
  summarise(correlation = cor(parasites_uL, count, use = "complete.obs"),
            p_value = cor.test(parasites_uL, count, method = "pearson")$p.value)

# Adjust p-values using the Benjamini-Hochberg method (FDR)
correlation_results_PE$adjusted_p_value <- p.adjust(correlation_results_PE$p_value, method = "fdr")

range(correlation_results_PE$adjusted_p_value)
range(correlation_results_PE$correlation)
