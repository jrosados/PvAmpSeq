...
# data_baseline <- data_all %>%
#   inner_join(samples_sum_PE %>% filter(day=="Day 0"), by = c("sample_id" = "sample"))  %>%
#  dplyr::select(sample_id, marker_id,sequence)

data_follow <- data_all %>%
  inner_join(samples_sum_PE %>% filter(day=="Day X"), by = c("sample_id" = "sample"))  %>%
  dplyr::select(sample_id, marker_id,sequence)
...
all_markers <- all_markers %>%
  filter(marker_id %in% as.character(data_follow$marker_id))
seq_tbl_all <- data_follow %>%
